...
set msh_patient_2 [regsub -all A17 $msh_patient_2 A02]
...
foreach segment $segments {
    ...
    switch $segmentID {
        "EVN" {
            set evn_segment_fields [split $segment $fieldSep]
            set evn_segment_fields [lreplace $evn_segment_fields 1 1 "A02"]
            #puts "EVN Segment Fields: $evn_segment_fields"
            set new_evn_segment [join $evn_segment_fields $fieldSep]
            #puts "EVN Segment: $new_evn_segment"
            lappend msg_patient_1 $new_evn_segment
            lappend msg_patient_2 $new_evn_segment            
            }
        "PID" {
            if {$loopFlag == 0} {
                lappend msg_patient_1 $segment
            } else {
                set pid_segment_fields [split $segment $fieldSep]
                #puts "PID Fields: $pid_segment_fields"
                set pid_segment_fields [lreplace $pid_segment_fields 1 1 "1"]
                #puts "Replace PID Fields: $pid_segment_fields"
                set new_pid_segment [join $pid_segment_fields $fieldSep]
                lappend msg_patient_2 $new_pid_segment
            }
        }
        "PV1" {
            if {$loopFlag == 0} {
                ...
                set pv1_segment_fields [split $segment $fieldSep]
                set pv1_segment_fields [lreplace $pv1_segment_fields 1 1 "1"]
                set new_pv1_segment [join $pv1_segment_fields $fieldSep]
                lappend msg_patient_2 $new_pv1_segment
            }
            incr loopFlag
        }
    }
...
set new_msg_patient_1 [join $msg_patient_1 \r]
set new_msg_patient_2 [join $msg_patient_2 \r]

set debugFlag 1

if $debugFlag {
    puts "\nPatient 1: $new_msg_patient_1"
    puts "\nPatient 2: $new_msg_patient_2"
}